...
<main class="single-ad-result">
  ...
  if (is_array($image) && isset($image['url'])) {
    echo '<img src="' . esc_url($image['url']) . '" alt="広告クリエイティブ画像" class="blurred-image" style="max-width:100%;height:auto;">';
  } elseif (is_numeric($image)) {
    echo '<img src="' . esc_url(wp_get_attachment_url($image)) . '" alt="広告クリエイティブ画像" class="blurred-image" style="max-width:100%;height:auto;">';
  } elseif (is_string($image)) {
    echo '<img src="' . esc_url($image) . '" alt="広告クリエイティブ画像" class="blurred-image" style="max-width:100%;height:auto;">';
  } else {
